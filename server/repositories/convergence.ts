...
/**
 * Legacy Prisma commerce → canonical schema convergence.
 *
 * `convergeLegacyData()` is the idempotent, all-or-nothing batch backfill
 * (production-readiness C5, step 1). `convergeAccount(id)` runs the identical
 * per-account logic for a single legacy account — the O(1) path used by the
 * live dual-write mirror (phase 3+). Both register every row in `legacy_map`
 * so re-runs never duplicate or orphan anything.
 */
...
function emptyReport(): ConvergenceReport {
  return {
    organizations: 0,
    customers: 0,
    subscriptions: 0,
...
async function loadVariants(c: PoolClient): Promise<Map<string, string>> {
  ...
  if (variants.size < 2) {
    throw new Error("Canonical catalog missing — run canonical:migrate first");
  }
  return variants;
}

/** Converge one legacy account within an open transaction. Idempotent. */
async function convergeOneAccount(
  c: PoolClient,
  account: AccountForConvergence,
  ...
  if (!orgId) {
    ...
    await mapPut(c, "accounts_org", account.id, "organizations", orgId, orgId);
  ...
  // 3) Subscriptions + items; status changes mirror onto already-mapped rows.
    ...
    if (!subId) {
      ...
      await c.query(
        `insert into public.subscription_items (organization_id, subscription_id, product_variant_id, qty, unit)
         values ($1,$2,$3,$4,'lb')`,
        [orgId, subId, variants.get(variantSkuFor(sub.variety)), sub.qtyLbs],
      );
      await mapPut(c, "subscriptions", sub.id, "subscriptions", subId, orgId);
      report.subscriptions++;
    } else {
      // Live-mirror lifecycle transitions (pause/resume/cancel/price/qty swap).
      await c.query(
        `update public.subscriptions
            set status=$2, paused_at=$3, cancelled_at=$4, price_cents=$5, plan=$6
          where id=$1`,
        [
          subId,
          status,
          ...
          sub.cancelledAt,
          sub.priceCents,
          sub.cadenceDays === 7 ? "WHOLESALE_STANDING" : sub.plan,
        ],
      );
      await c.query(
        `update public.subscription_items set qty=$2, product_variant_id=$3
          where subscription_id=$1`,
        [subId, sub.qtyLbs, variants.get(variantSkuFor(sub.variety))],
      );
    }

    // 4) Cycles → subscription_deliveries
    for (const cycle of sub.cycles) {
      const existing = await mapGet(c, "cycles", cycle.id);
      const status = cycle.status.toLowerCase(); // enum sets match by design
      if (existing) {
        await c.query(
          `update public.subscription_deliveries set status=$2, confirmed_at=$3 where id=$1`,
          [existing, status, cycle.confirmedAt],
        );
        continue;
      }
      const row = await c.query(
...
async function auditOrphans(c: PoolClient, report: ConvergenceReport) {
...
export async function convergeLegacyData(): Promise<ConvergenceReport> {
  ...
  try {
    await c.query("begin");
    const variants = await loadVariants(c);
    ...
    for (const account of accounts) {
      await convergeOneAccount(c, account, variants, report);
    }
    await auditOrphans(c, report);
...
/** Converge a single legacy account (live dual-write path). Idempotent. */
export async function convergeAccount(
  accountId: string,
