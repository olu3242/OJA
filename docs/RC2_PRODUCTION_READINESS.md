...
The core platform is real and tested end-to-end at the service layer (94
...
surface a pilot needs: **health/readiness/liveness probes**, **baseline
security headers**, and **webhook idempotency + retry primitives**. The conditions blocking an unconditional GO are all
...
| WS9 Reliability + WS2 idempotency | `lib/idempotency.ts` (`withIdempotency`, PG-backed `webhook_events` ledger, additive migration) + `lib/retry.ts` (`withRetry` backoff, `withTimeout`). POS webhook now dedupes replays. | HTTP replay returns `replayed:true` with **1** SELL_THROUGH event, not 2; 7 tests |
| WS11 QA                           | `tests/integration/{health,reliability}.test.ts` (12 tests) added to the suite (82 → 94)                                                                                                | `vitest run` 94 passing                                                           |
...
| 2   | Commerce E2E      | PARTIAL             | Subscription/group/wholesale/renewal/cancel flows covered by integration tests through the real services. **DONE this session:** webhook idempotency (`withIdempotency` + `webhook_events` ledger) — the POS webhook now dedupes duplicate deliveries; the same seam is ready for Stripe events. **GAP:** in-browser E2E of the full purchase path is blocked by auth being EXTERNAL and payment being a stub (Stripe adapter not yet implemented). |
...
| 9   | Reliability       | PARTIAL             | **DONE this session:** `withIdempotency` + `webhook_events` replay ledger (duplicate-webhook protection, verified over HTTP); `withRetry` (exponential backoff, injectable sleep) + `withTimeout`. Existing: `mirrorAccount` best-effort (never breaks the revenue path); convergence idempotent + parity-gated; hourly reconcile job. **GAP:** circuit breakers + dead-letter queue; wiring `withRetry` around the carrier/payment adapters.       |
...
| Webhook idempotency      | ✅           | HTTP replay → `replayed:true`, 1 event not 2; 7 reliability tests    |
